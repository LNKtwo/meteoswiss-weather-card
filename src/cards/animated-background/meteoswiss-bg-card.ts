...
export class MeteoSwissBGCard extends LitElement {
  ...
  public getGridOptions() {
    const rows = this.config?.grid_options?.rows ?? 4;
    return {
      rows: rows,
  ...
  public static getStubConfig() {
    return {
      ...
      entity: '',
      scenery_image:
        'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c0/Vierwaldst%C3%A4ttersee_with_Mountain_Panorama.jpg/1280px-Vierwaldst%C3%A4ttersee_with_Mountain_Panorama.jpg',
    };
  }
  ...
  public render(): TemplateResult {
    ...
    const gridRows = this.config?.grid_options?.rows ?? 4;

    // Set CSS variable for card height calculation
    this.style.setProperty('--card-grid-rows', gridRows.toString());

    // Auto-resolve MeteoSwiss sensors
    if (!this._resolvedEntities || this._lastEntityId !== this.config.entity) {
      this._resolvedEntities = resolveMeteoSwissEntities(this.hass, this.config.entity);
    }
    ...
    if (!weatherEntity || !weatherEntity.attributes) {
      return html`<div class="bg-card-root"><div style="padding:20px;text-align:center;color:var(--secondary-text-color);">Entity not found: ${this.config.entity}</div></div>`;
    }
    // Parse temperature: try weather entity attribute first, then auto-resolved sensor
    const rawTemp = weatherEntity.attributes.temperature;
    let temperature = NaN;
    ...
    // Fallback: use auto-resolved temperature sensor
    if (isNaN(temperature) && this._resolvedEntities?.temperature) {
      temperature = getNumber(this.hass, this._resolvedEntities.temperature) ?? NaN;
    }
    ...
    return html`
      <div class="bg-card-root">
        <div style="position:absolute;top:8px;left:12px;z-index:20;display:flex;align-items:center;justify-content:center;border-radius:45px;border:2px solid var(--primary-text-color,#fff);background:rgba(0,0,0,0.45);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);padding:5px 12px;box-shadow:0 2px 8px rgba(0,0,0,0.4);font-size:${fontSizePx};font-weight:bold;color:#fff;text-shadow:0 1px 3px rgba(0,0,0,0.8);">
          ${!isNaN(temperature) ? Math.round(temperature) : '--'}°
        </div>
        ${
          condition
            ? html`${
                ...
                ${
                  day && this.config.show_day_temps !== false
                    ? html`
                        <div style="position:absolute;top:calc(${fontSizePx} + 52px);left:12px;z-index:20;font-size:14px;color:#fff;display:flex;flex-direction:row;gap:10px;text-shadow:0 1px 4px rgba(0,0,0,0.9),0 0 2px rgba(0,0,0,0.8);">
                ...
                ${
                  sunEntityId && this.config.show_sun_times !== false && condition !== 'clear-night'
                    ? html`
                        <div style="position:absolute;top:calc(${fontSizePx} + 52px);right:16px;z-index:20;display:flex;gap:12px;align-items:center;color:#fff;font-size:14px;text-shadow:0 1px 4px rgba(0,0,0,0.9),0 0 2px rgba(0,0,0,0.8);">
                ...
                <div style="position:absolute;top:14px;right:16px;z-index:20;font-size:16px;color:#fff;text-align:right;text-shadow:0 1px 4px rgba(0,0,0,0.9),0 0 2px rgba(0,0,0,0.8);">${_t(condition)}</div> `
            : html``
        }
      </div>
    `;
  ...
  private _renderPhotoLikeBackground(condition: string, daytime: boolean): TemplateResult {
    ...
    const configScenery = (this.config as any).scenery_image;
    // Use configured image, or fall back to default Swiss lake panorama when photo_mode is on
    const sceneryImage = configScenery !== undefined
      ? configScenery
      : (this.config as any).photo_mode === true
        ? 'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c0/Vierwaldst%C3%A4ttersee_with_Mountain_Panorama.jpg/1280px-Vierwaldst%C3%A4ttersee_with_Mountain_Panorama.jpg'
        : '';

    return html`
      <div
        class="img-photo ${sceneryImage ? 'has-scenery' : ''} mood-${mood} ${
          isNightVisual ? 'night' : 'day'
        } ${isClearNight ? 'clear-night' : ''} ${isFog ? 'foggy' : ''} ${
          isPartlyCloudy ? 'partly-cloudy' : ''
        ...
        ${
          sceneryImage
            ? html`<div
                ...
                style="background-image: url('${sceneryImage}');"
              ></div>`
            : html``
        }
