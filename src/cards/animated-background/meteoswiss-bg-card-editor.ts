...
@customElement(ANIMATED_BACKGROUND_CARD_EDITOR_NAME)
export class MeteoSwissBGCardEditor extends LitElement implements LovelaceCardEditor {
  ...
  protected render(): TemplateResult {
    ...
    const data = {
      ...
      scenery_image:
        typeof (this._config as any)?.scenery_image === 'string'
          ? (this._config as any).scenery_image
          : undefined,
      forecast_mode:
        ...
        <div class="group">
          ...
          <ha-form
            ...
            .schema=${[
              ...
              schema.find(s => s.name === 'scenery_image'),
            ].filter(Boolean)}
            .computeLabel=${this._computeLabel}
            .computeHelper=${this._computeHelper}
            @value-changed=${this._valueChanged}
  ...
  private _computeLabel = (schema: any) => {
    const labels: Record<string, string> = {
      ...
      scenery_image: _t('bg_card.config.scenery_image') || 'Scenery Image URL',
    };
  ...
  private _renderConfigPreview(): string {
    ...
    if (!config.type) {
      config.type = `custom:${ANIMATED_BACKGROUND_CARD_NAME}`;
    }
